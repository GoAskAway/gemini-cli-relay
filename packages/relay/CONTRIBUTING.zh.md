...
我们的同步工作流会：
1. **定期检查**上游发布版本：
   - 每周三 14:00（北京时间）- 主要检查时间
   - 每天 20:00（北京时间）- 日常检查
2. **发布版本同步** - 只同步稳定发布版本，不同步开发中间状态
3. **智能创建 PR** 当检测到新发布版本时
4. **清理旧 PR** 避免多个同步 PR 并存

**注意**: 我们只同步上游的稳定发布版本（如 v0.1.22），不同步 nightly、preview 或开发中的 commit，确保 `upstream-main` 分支的稳定性。
